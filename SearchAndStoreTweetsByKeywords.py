...
class Messages(Base):
    ...
    def __init__(self, query, tweet_id, inserted_date, truncated, language, possibly_sensitive, coordinates,
                 retweeted_status, created_at_text, created_at, content, from_user_screen_name, from_user_id,
                 from_user_followers_count, from_user_friends_count, from_user_listed_count, from_user_statuses_count,
                 from_user_description, from_user_location, from_user_created_at, retweet_count, entities_urls,
                 entities_urls_count, entities_hashtags, entities_hashtags_count, entities_mentions,
                 entities_mentions_count, in_reply_to_screen_name, in_reply_to_status_id, source,
                 entities_expanded_urls, json_output, entities_media_count, media_expanded_url, media_url, media_type,
                 video_link, photo_link, twitpic
                 ):
        self.query = query
...
def get_data(kid, max_id=None):
    try:
        d = t.search(q=kid, count='100', result_type='recent',
                     # lang='zh',
                     lang='en',
...
def write_data(self, d):
    ...
    for entry in d['statuses']:
        ...
        # video_link = 0
        if 'vimeo' in entities_expanded_urls \
                or 'youtube' in entities_expanded_urls \
                or 'youtu' in entities_expanded_urls \
                or 'vine' in entities_expanded_urls:
            video_link = 1					
            print "FOUND A VIDEO!!!"
        else:
            video_link = 0
        ...
        if 'twitpic' in entities_expanded_urls:
            ...
            twitpic = 0
        if 'twitpic' in entities_expanded_urls \
                or 'instagram' in entities_expanded_urls \
                or 'instagr' in entities_expanded_urls:
            photo_link = 1					
        ...
        if not updates:
            ...
            upd = Messages(query, tweet_id, inserted_date, truncated, language, possibly_sensitive,
                           ...
                           from_user_description, from_user_location, from_user_created_at, retweet_count,
                           entities_urls, entities_urls_count, entities_hashtags, entities_hashtags_count,
                           entities_mentions, entities_mentions_count, in_reply_to_screen_name, in_reply_to_status_id,
                           source, entities_expanded_urls, json_output, entities_media_count, media_expanded_url,
